...
- `styles/themes/`：主题级 CSS 模式，例如 island 演示组合样式

结构层组件负责内容组织，主题层组件负责视觉表达，`styles/themes/` 承载纯 CSS 的主题演示模式。`weekly/_current/` 只保存当前内容，不作为样式系统入口。新主题可以复用相同的内容工作流，而不需要重写整套幻灯片结构。
